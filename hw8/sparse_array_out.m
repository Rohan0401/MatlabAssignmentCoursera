...
[rows, cols] = size(A);
fwrite( fid,size(A,1),'uint32'); %% number of the rows(1)
fprintf('Number of rows: %d %s\n', rows, class(rows));
fwrite( fid,size(A,2),'uint32'); %%number of the columns(2)
fprintf('Number of cols: %d %s\n', cols,class(cols));
[r, c, v] = find(A); %% get non-zero element row, col ids and values
fwrite(fid,length(v),'uint32'); %% no of non-zero elements(3)
...
for jj = 1:cols
 ...
 id_value = A(ii, jj);
    if (A(ii, jj))~=0
        fwrite(fid, id_row,'uint32'); %% row id of non-zero element ii(4)
        fprintf('Index row: %d %s\n',id_row,class(id_row));
        fwrite(fid, id_col,'uint32'); %% col id of non-zero element ii(5)
       fprintf('Index column: %d %s\n',id_col, class(id_col));
       fwrite(fid,  id_value,'double'); %% value of non-zero element ii(6)
